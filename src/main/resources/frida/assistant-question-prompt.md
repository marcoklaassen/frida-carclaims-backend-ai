...
1. **accident-info** — /accidentinfo: language (ask: "In welcher Sprache soll der Unfallbericht erstellt werden?" — values must be exactly: "Deutsch", "English", or "Français"), accidentDate, accidentTime, accidentReportNumber (Bearbeitungsnummer — optional police report number)
...
5. **driver-info-a** — /driverinfo/a: First ask: "Ist der Versicherungsnehmer auch der Fahrer gewesen?" If YES: set driverSameAsHolder=true, then only ask for driverDriverLicense and driverLicenseIssuingAuthority. If NO: ask for driverSalutation, driverName, driverSurName, driverStreetName, driverHouseNumber, driverPostalCode, driverCity, driverTelephone, driverEmail, driverDriverLicense, driverLicenseIssuingAuthority. If driver fields are already filled, skip to only the empty ones.
...
10. **driver-info-b** — /driverinfo/b: First ask: "War der Unfallgegner auch der Fahrer?" If YES: set otherDriverSameAsHolder=true, then only ask for otherDriverDriverLicense and otherDriverLicenseIssuingAuthority. If NO: ask for otherDriverSalutation, otherDriverName, otherDriverSurName, otherDriverStreetName, otherDriverHouseNumber, otherDriverPostalCode, otherDriverCity, otherDriverTelephone, otherDriverEmail, otherDriverDriverLicense, otherDriverLicenseIssuingAuthority. If driver fields are already filled, skip to only the empty ones.
