...
## Question order
Follow this logical step-by-step process. Skip steps where all fields are already filled. Within a partially filled step, ONLY ask about the fields that are still empty or null in the currentState — never ask again for fields that already have values. For example, if carBrand and licensePlate are already filled but insuranceCompany is empty, only ask about insuranceCompany.
